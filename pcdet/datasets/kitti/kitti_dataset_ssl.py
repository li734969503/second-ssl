...
class KittiDatasetSSL(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.use_exchange = self.dataset_cfg.get('EXCHANGE', False)
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            index_unlabeled = np.random.choice(len(self.unlabeled_index_list), 1)[0]
            ...
            #print(data_dict_labeled)
            #print(data_dict_unlabeled)
            if self.use_exchange:
                self.exchange(data_dict_labeled,data_dict_unlabeled)
            return [data_dict_labeled, data_dict_unlabeled]
        else:
            return data_dict_labeled

    def exchange(self, labeled,unlabeled):
        labeled['points_ema'] = unlabeled['points']
        labeled['voxel_coords_ema'] = unlabeled['voxel_coords']
        labeled['voxels_ema'] = unlabeled['voxels']
        labeled['voxel_num_points_ema'] = unlabeled['voxel_num_points']
        labeled['gt_boxes_ema'] = unlabeled['gt_boxes']
    @staticmethod
    ...
    def prepare_data(self, data_dict, no_db_sample=False):
        ...
        if self.training:
            ...
            gt_boxes_ema, points_ema = global_scaling(gt_boxes_ema, points_ema, [0, 2], False,
                                                         scale_=1/data_dict['noise_scale'])
            gt_boxes_ema, points_ema = global_rotation(gt_boxes_ema, points_ema, [-1, 1], False,
                                                          rot_angle_=-data_dict['noise_rot'])
            gt_boxes_ema, points_ema = random_flip_along_x(gt_boxes_ema, points_ema, enable_=data_dict['flip_x'])
            #gt_boxes_ema, points_ema = random_flip_along_y(gt_boxes_ema, points_ema, enable_=data_dict['flip_y'])
